...
glm::mat4 GLCamera::GetViewMatrix()
{
	...
	{
		glm::vec3 tempPos = *positionToFollow;
		tempPos.y += 10;
		return glm::lookAt(Position, *positionToFollow + glm::vec3(0, 20, 0), glm::vec3(0, 1, 0));
	}
}
...
void GLCamera::ProcessKeyboard(Camera_Movement direction, double deltaTime)
{
	if (mode == FIRSTPERSONMODE)
	{
...
void GLCamera::ProcessJoystickMovement(float xDir, float yDir, float Velocity, double deltaTime)
{
